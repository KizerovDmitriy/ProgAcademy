...
public class MainProgramApartments {
        ...
        try (Connection connection = ConnectionFactory.getConnection("db.properties");
            ...
            while (true) {
                ...
                switch (choice) {
                    ...
                    case "1":
                        addApartmentsCase(scanner, dao);
                        break;

                    case "2":
                        viewApartmentsCase(dao);
                        break;

                    case "3":
                        if (getAllCase(scanner, dao)) return;
                        break;
                    default:
                        return;
                }
    ...
    private static boolean getAllCase(Scanner scanner, ApartmentsDAO dao) {
        ...
        if (filterCase(dao, filterNumber, filter)) return true;
        return false;
    }

    private static void viewApartmentsCase(ApartmentsDAO dao) {
    ...
    private static boolean filterCase(ApartmentsDAO dao, String filterNumber, String filter) {
        switch (filterNumber) {
            case "1":
    ...
    private static void addApartmentsCase(Scanner scanner, ApartmentsDAO dao) {
